...
def search_i(text,chat,bot):
    try:
        ...
        if hasInsult(search)[0] == False and chat['abs_object']['type'] == 'Group':
            bot.sendMessage(chat['object_guid'], 'Full results will be sent to your pew!👨🏻‍💻', chat['last_message']['message_id'])                           
            ...
            bot.sendMessage(chat['object_guid'], 'Be patient finding ••• ', chat['last_message']['message_id'])
            print('search image')
            jd = json.loads(requests.get('https://zarebin.ir/api/image/?q=' + search + '&chips=&page=1').text)
            jd = jd['results']
            a = 0
    ...
    try:
        ...
        if user_info['data']['exist'] == True:
            if user_info['data']['type'] == 'User':
                ...
                bot.sendMessage(chat['object_guid'], 'Is a channel!' , chat['last_message']['message_id'])
                print('sended response')
        else:
            bot.sendMessage(chat['object_guid'], 'Does not exist!' , chat['last_message']['message_id'])
            print('sended response')
        return True
    except:
        print('server bug6')
    ...
    try:
        ...
        if hasInsult(search)[0] == False and chat['abs_object']['type'] == 'Group':                               
            ...
            for result in results:
                text += result['title'] + '\n\n'
            bot.sendMessage(chat['object_guid'], 'Results sent to your pew', chat['last_message']['message_id'])
        ...
        bot.sendMessage(chat['object_guid'], 'دThis command is currently restricted or under repair' , chat['last_message']['message_id'])
        return False
...
def get_added(text,chat,bot):    
    try:
        ...
        send_text = 'Hello to '+ group +' Welcome, please follow the rules--> @heruyny ☄️'
        bot.sendMessage(chat['object_guid'],  send_text, chat['last_message']['message_id'])
    except:
        print('rub server err')
                    ...
                    if 'SendMessages' in access and chat['last_message']['type'] == 'Text' and text.strip() != '':
                        ...
                        if not m_id in list_message_seened:
                            ...
                            if text == '!start':
                                ...
                                try:
                                    bot.sendMessage(chat['object_guid'], 'Hello, Welcome to Hosein Naaji(@heruyny) Robot. Please send the word (!راهنما).',chat['last_message']['message_id'])
                                    print('sended response')    
                                except:
                                    print('server bug1')
                                ...
                                tawd27.start()
                            elif text.startswith('nskjsndn!search-k ['):
                                ...
                                try:
                                    ...
                                    print('ban bug')
                            elif text.startswith('!سرچ عکس [kwjdjdbknwnsjsj'):
                                print('mpa started')
                                ...
                                tawd2.start()
                            elif text.startswith('!ترجمه [kdkdjjd'):
                                tawd28 = Thread(target=get_trans, args=(text, chat, bot,))
                                tawd28.start()
                            elif text.startswith('!mykeigfgt-s ['):
                                try:
                                    search = text[10:-1]
                                    if hasInsult(search)[0] == False and chat['abs_object']['type'] == 'Group':
                                ...
                                tawd14.start()
                            elif text.startswith('!راهنماkjjvjjdj'):
